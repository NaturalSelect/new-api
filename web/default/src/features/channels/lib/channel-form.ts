...
export const channelFormSchema = z
  .object({
    ...
    auto_ban_optimistic: z.number().optional(),
    auto_unban_by_balance: z.number().optional(),
    status: z.number(),
...
export function transformChannelToFormDefaults(
  ...
  return {
    ...
    auto_ban_optimistic: channel.auto_ban_optimistic ?? 0,
    auto_unban_by_balance: channel.auto_unban_by_balance ?? 1,
...
function buildSettingJSON(formData: ChannelFormValues): string {
  const freeModels = Array.from(
    new Set(
      String(formData.free_models_list || '')
        .split(',')
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  if ([1, 14, 58, 59].includes(formData.type)) {
    settingsObj.auto_cache_control = formData.auto_cache_control === true
  } else {
    if ('auto_cache_control' in settingsObj) delete settingsObj.auto_cache_control
  }
...
export function transformFormDataToCreatePayload(formData: ChannelFormValues): {
  ...
  const channel: Partial<Channel> = {
    ...
    auto_ban_optimistic: formData.auto_ban_optimistic ?? 0,
    auto_unban_by_balance: formData.auto_unban_by_balance ?? 1,
