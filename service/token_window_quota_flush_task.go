...
// periodically snapshots every active token's 5h/7d quota window into the
	...
	tokenWindowQuotaFlushOnce.Do(func() {
		...
		if err != nil {
			...
			for _, skip := range result.Skips {
				if skip.Failed {
					logger.LogWarn(ctx, fmt.Sprintf("token window quota restore failed for token=%d window=%s window_start=%d: %s", skip.TokenId, skip.Window, skip.WindowStart, skip.Reason))
				} else {
					logger.LogDebug(ctx, fmt.Sprintf("token window quota restore skipped token=%d window=%s window_start=%d: %s", skip.TokenId, skip.Window, skip.WindowStart, skip.Reason))
				}
			}
		}
